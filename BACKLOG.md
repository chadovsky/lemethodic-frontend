...
**Status:** Built locally, committed to `main`, **holding push for Chadi** (FE auto-deploys Vercel). After push + CI green, prod spot-check `/tableau-de-bord` (three zones, hero links to /carte, no 375 overflow, sits correctly beside the icon rail), then a follow-up `docs(F-464): flip status to Shipped (<SHA>)` records the SHA. Layout locked with Chadi against the Nano Banana dashboard mock. Depends on F-463 (palette) + F-465 (icon rail), both shipped to main.
