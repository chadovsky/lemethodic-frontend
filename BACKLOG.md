...
**Status:** Built, all gates green, **holding for Chadi's push** (direct-to-main, no remote push yet). Full unit suite 609/609 green; `pnpm build` green (clean `.next`); carte e2e 14/14 against the prod build (`pnpm start`); F-225 receipts captured + eyeballed. Visual is tunable against the Nano Banana scatter mock via the authored coords in `lib/carte/layout-map.ts` (Chadi nudges, no geometry change).
