...
- **`app/globals.css` (F-483 block):** coral ramp tokens `--couche-ramp-1..5`, `--couche-skew (-14deg)`, `--couche-depth`, `--couche-lift`, and the `.couche-stack*` styles -- the wrapper skewY tilt; each layer's two depth faces via `::before` (left, skewY 45deg origin right) + `::after` (top, skewX 45deg origin bottom), darker-shaded with `color-mix(... black)`; per-layer z-index (+ a `:has()` hover bump); counter-skewed content; grid-rows detail collapse; hover/`:focus-visible` lift + reveal; `@media (hover:none)`, `@media (max-width:640px)` flat fallback (drop skew + faces, full width, details open, no overflow), and a `prefers-reduced-motion` guard. uiverse MIT attribution included. Only new hex is the coral ramp; depth faces use the `black` keyword in `color-mix`, text uses the `black`/`white` keywords (mode-stable).
...
**AA contrast (resolved):** small text (eyebrow + detail) clears 4.5:1 on every block. Upper three blocks take dark text (10.8 / 7.3 / 5.0:1), lower two take light text (5.2 / 7.3:1). Chadi can flip to all-dark-with-lighter-deep-steps on the preview if he prefers; both directions stay >=4.5:1.
...
**Corrective (this session):** v1 applied only the skewY tilt with a single flat coral gradient and no depth faces, so it read as a flat tilted plane. Rebuilt as a true isometric 3D stack: the two pseudo-element depth faces per layer, per-layer z-index, a stepped (AA-tuned) ramp with darker side faces, increased depth, and the heading-order fix. Same branch.
