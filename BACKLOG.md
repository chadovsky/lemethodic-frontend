...
**Scope (current = uiverse skewed stack, Chadi's exact technique):** A `<ul class="couche-stack">` tilted with `skewY(-14deg)`; each `<li>` is an extruded block -- `::before` = left face (`skewY 45deg`), `::after` = top face (`skewX 45deg`), 20px depth. Lower couches sit in front (z-index 1..5) so no label is covered. The DM Mono eyebrow + Inter extrabold phrase ride the stack skew, so the type lies ON the isometric plane. Each face is a blue tone from the sky tokens -- no white/transparent. The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.
...
- **`app/globals.css` (F-483 block):** the sky colour tokens (`--couche-top/ramp/side-1..5` + `--couche-ink`) plus the skewed-stack CSS exactly per the brief -- `.couche-viz` (centered, entrance `coucheRise`), `.couche-stack` (`skewY(-14deg)`), `.couche-stack li` (360px, min-height 70, `z-index: var(--i)`, `background: var(--front)`, staggered `coucheFade`), `li::before`/`li::after` 20px depth faces (`var(--side)`/`var(--top)`), `li:hover { translateX(-20px) }`, `.couche-ey` (DM Mono, wired to `var(--f-mono)`) + `.couche-ph` (Inter 800, `var(--f-en)`). Two adaptations to our stack: fonts/ink wired to our tokens (not literal `'Inter'`/`#1F2937`); `.couche-viz { overflow-x: clip }` so the left face never adds horizontal scroll on narrow viewports; and a `prefers-reduced-motion` guard re-asserting `opacity: 1` (the global rule sets `animation-name: none`, which would otherwise leave the `li` opacity:0 base hidden).
...
- *Corrective 6 (this PR -- uiverse skewed stack):* replaced the SVG with Chadi's exact uiverse skewed-stack component (CSS `skewY` stack + `::before`/`::after` 3D faces, `li:hover` translateX). Wired our strings + sky tokens + fonts. Two stack adaptations: `overflow-x: clip` on `.couche-viz` (the 360px li + left face would otherwise add ~12px horizontal scroll at 375 -- this clips ~12px of the left face on mobile, flagged) and a reduced-motion `opacity:1` guard. Mobile keeps readable 16px text (better than the SVG's scaled-down text). NOTE: this technique uses a fixed 360px li width, so there is no rightward fan (uniform-width skewed stack); the block is centered.
