...
- *Corrective 7 (this PR -- overlap geometry):* per the geometry-fix brief -- per-block top faces removed; real overlap added (`li + li { margin-top: -22px }`) so each couche hides the bottom of the one above (this is what makes it read as a stack); top face kept only on the first couche; depth 20px -> 22px (left face + first-child top face in sync); min-height 70 -> 72; content top-aligned so the hidden strip stays empty. Kept the three stack adaptations.
