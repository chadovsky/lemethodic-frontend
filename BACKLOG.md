...
## F-483 -- FE: 5-couche methodology visualizer on /la-methode (isometric stack, sky)

**Status:** Built on `feat/f483-couche-visualizer-sky`. PR opened against `main`; NOT merged -- Chadi judges on the Vercel preview and confirms before any squash-merge. Local gates: full unit suite (incl. `CoucheStack` suite, 6); clean `pnpm build` (fresh `rm -rf .next`); f-483 + ecole-list e2e green. F-225 receipts `tests/screenshots/f-483-la-methode-{1440,375}.png` + trace `f-483.zip` (gitignored).
...
**Scope (current = sky reskin):** An isometric stack (the uiverse skewY effect, by andrew-demchenk0, MIT) showing the five couches as one flush, contiguous extruded block, reskinned to a sky ramp. Each layer shows a DM Mono uppercase couche eyebrow above an Inter extrabold How-to phrase; both inherit the bar skew, so the type slants WITH the box and sits ON the isometric plane (like the reference). Each layer is a three-tone blue block: a top-lit front (gradient top -> front), a darker side/depth face, and a lighter top face on the first layer -- no white or transparent face anywhere. Layers are flush (gap 0), lower layers in front (so no label is occluded); rounded 7px corners. The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.

**Built:**
- **`components/la-methode/CoucheStack.tsx`:** section h2 + intro, then a `role="list"` of five blocks. Each block is driven from the single `COUCHES` array (eyebrow + phrase + top/front/side tone tokens) -- no raw hex in the markup -- setting `--face-top` + `--face-front` + `--face-side`; the `<li>` sets `--i` (z-index, layer 1 lowest -> layer 5 highest, so lower layers sit in front). Eyebrow (`couche-stack-eyebrow`) + `<h3>` phrase (`couche-stack-headline`) sit in a `.couche-layer__content` wrapper that inherits the bar skew (text on the plane).
...
- **`app/layout.tsx`:** added Inter weight `800` (extrabold) for the phrases; DM Mono was already wired (`--f-mono`).
- **`app/globals.css` (F-483 block):** centralized sky tokens -- `--couche-top-1..5` (top tones) + `--couche-ramp-1..5` (front tones) + `--couche-side-1..5` (side/depth tones) + `--couche-ink (#1F2937)`; `--couche-skew (-14deg)`, `--couche-depth`, `--couche-lift`. `.couche-stack` is `gap: 0` (flush). Each layer front face is `linear-gradient(180deg, var(--face-top), var(--face-front))` (top-lit, all blue), ink text, `border-radius: 7px`. Depth faces `::before` (left, skewY 45deg origin right) = `var(--face-side)`; `::after` (top, skewX 45deg origin bottom, FIRST layer only) = `var(--face-top)`; both 7px rounded. `.couche-layer__content` has NO counter-skew (text inherits the bar skew, on the plane). Eyebrow = DM Mono uppercase, 0.14em tracking, ~10.5px; phrase = Inter 800. `@media (max-width:640px)` flat upright fallback (stack un-skewed -> text upright, drop faces, full width). uiverse MIT attribution kept.
...
**Typography (LOCKED):** phrases Inter 800 (extrabold); eyebrows DM Mono, uppercase, ~0.14em tracking, ~10.5px. Text slants on the plane (inherits the bar skew).
...
- *Corrective 4 (this PR):* (1) text back ON the plane -- removed the inverse-skew wrapper so eyebrow + phrase slant with the box; (2) three-tone all-blue faces -- front is a top-lit gradient (top -> front), side/depth + first-layer top face are blue tones, no white/transparent face anywhere (there was no inset-white box-shadow in the code; the perceived white was the pale top tint, now an explicit blue gradient); (3) z-index confirmed lower-layers-in-front so no label (e.g. La Construction) is occluded.
