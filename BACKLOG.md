...
**Scope (current = sky reskin):** An isometric stack (the uiverse skewY effect, by andrew-demchenk0, MIT) showing the five couches as one flush, contiguous extruded block, reskinned to a sky ramp. Each layer shows a DM Mono uppercase couche eyebrow above an Inter extrabold How-to phrase; both ride a counter-skewed wrapper so the type renders UPRIGHT while the bars keep their skew + rightward fan. Layers are flush (gap 0); rounded 7px corners. The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.

**Built:**
- **`components/la-methode/CoucheStack.tsx`:** section h2 + intro, then a `role="list"` of five blocks. Each block is driven from the single `COUCHES` array (eyebrow + phrase + front-fill token + side-shade token) -- no raw hex in the markup -- and sets `--face-top` + `--face-side`; the `<li>` sets `--i` (z-index). Eyebrow (`couche-stack-eyebrow`) + `<h3>` phrase (`couche-stack-headline`) sit in a `.couche-layer__content` upright wrapper.
...
- **`app/globals.css` (F-483 block):** centralized sky tokens `--couche-ramp-1..5` (front fills) + `--couche-side-1..5` (per-layer side shades) + `--couche-ink (#1F2937)`; `--couche-skew (-14deg)`, `--couche-depth`, `--couche-lift`. `.couche-stack` is `gap: 0` (flush). Each layer: front fill `var(--face-top)`, ink text, `border-radius: 7px`, and a bottom-shelf padding = `--couche-depth` so the layer below's top face lands on the shelf and never covers the text (last layer trims the shelf). Depth faces `::before` (left, skewY 45deg origin right) + `::after` (top, skewX 45deg origin bottom) use `var(--face-side)`, also 7px rounded. `.couche-layer__content` carries the inverse skew (upright text). Eyebrow = DM Mono uppercase, 0.14em tracking, ~10.5px; phrase = Inter 800. `@media (max-width:640px)` flat upright fallback (drop skew + faces, full width). uiverse MIT attribution kept.

**Colour (LOCKED, sky ramp):** front / side per layer -- Le Propos `#E8F6FE / #C8E9FC`; Le Plan `#C7EBFC / #A3DCF8`; La Construction `#93DAF9 / #66C7F1`; Les Pièges Anglais `#54C3F2 / #2CA8E5`; La Musique `#1FAEE8 / #0C8FC9`. Coral removed from THIS surface only; the global `--accent` coral token is untouched.

**Ink + AA (LOCKED):** every layer uses one dark ink `#1F2937` for both eyebrow and phrase -- zero white text. All five sky fills are light enough that dark ink clears 4.5:1 (deepest fill `#1FAEE8` vs `#1F2937` ~ 5.8:1).

**Typography (LOCKED):** phrases Inter 800 (extrabold); eyebrows DM Mono, uppercase, ~0.14em tracking, ~10.5px.
...
- *Corrective 3 (this PR -- sky):* sky ramp (front + side) + single dark ink, flush (gap 0), rounded 7px, phrases Inter 800, couche eyebrows re-added in DM Mono uppercase, and text returned to UPRIGHT via the counter-skewed wrapper.
