...
## F-483 -- FE: 5-couche methodology visualizer on /la-methode (uiverse skewed stack, sky)

**Status:** Built on `feat/f483-couche-visualizer-sky`. PR #17 against `main`; NOT merged -- Chadi judges on the Vercel preview and confirms before any squash-merge. Local gates: full unit suite 587/587 (incl. `CoucheStack` suite, 5); clean `pnpm build` (fresh `rm -rf .next`); f-483 + ecole-list + reduced-motion e2e green; `tsc` clean for the component. F-225 receipts `tests/screenshots/f-483-la-methode-{1440,375}.png` + trace `f-483.zip` (gitignored).

**Branch base note:** the F-483 component lives only on the prior unmerged branch `feat/f-483-couche-visualizer` (`d9bf469`), which sits directly on current `main` (`6472f8a`) -- NOT stale. The brief said "branch off fresh main," but main has no component to reskin, so `feat/f483-couche-visualizer-sky` was branched off the prior F-483 branch (= current main + the feature). The PR base is `main`, so the PR diff is the COMPLETE feature in its sky form. The prior coral branch is superseded by this one.
...
**Scope (current = uiverse skewed stack, Chadi's exact technique):** A `<ul class="couche-stack">` tilted with `skewY(-14deg)`; each `<li>` is an extruded block with a left face (`::before`, `skewY 45deg`, 22px depth). The couches really overlap (`li + li { margin-top: -22px }`), so each one hides the bottom strip of the one above and the stack reads as a stack; content is top-aligned so the hidden strip stays empty. The top face (`::after`, `skewX 45deg`) is kept ONLY on the first couche (the unambiguous top of the stack). Lower couches sit in front (z-index 1..5) so no label is covered. The DM Mono eyebrow + Inter extrabold phrase ride the stack skew, so the type lies ON the isometric plane. Every face is a blue tone from the sky tokens -- no white/transparent. The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.

**Built:**
- **`components/la-methode/CoucheStack.tsx`:** section h2 + intro, then `<ul class="couche-stack" role="list">` of five `<li>`. Each `<li>` is driven from the `COUCHES` array -- it sets `--i` (z-index) + `--front`/`--side`/`--top` to the sky tokens (no raw hex in the markup) -- and holds a `<span class="couche-ey" data-testid="couche-stack-eyebrow">` + `<span class="couche-ph" data-testid="couche-stack-headline">`.
- **Heading-order:** `LessonList` + `LessonListContainer` carry an optional `methodology` slot rendered between the page h1 header and the lesson sections; `app/(shell)/la-methode/page.tsx` passes `<CoucheStack />` into it. The h1 + subtitle stay in `LessonList` (ecole-list e2e unchanged).
- **`app/layout.tsx`:** Inter weight `800` (extrabold) for the phrases; DM Mono already wired (`--f-mono`).
- **`app/globals.css` (F-483 block):** the sky colour tokens (`--couche-top/ramp/side-1..5` + `--couche-ink`) plus the skewed-stack CSS per the brief -- `.couche-viz` (centered, entrance `coucheRise`), `.couche-stack` (`skewY(-14deg)`), `.couche-stack li` (360px, min-height 72, `z-index: var(--i)`, `background: var(--front)`, staggered `coucheFade`), `li + li { margin-top: -22px }` (the real overlap), `li::before` left face + `li:first-child::after` top face (22px depth, `var(--side)`/`var(--top)`), `li:hover { translateX(-20px) }`, `.couche-ey` (DM Mono, wired to `var(--f-mono)`) + `.couche-ph` (Inter 800, `var(--f-en)`). Three adaptations to our stack: fonts/ink wired to our tokens (not literal `'Inter'`/`#1F2937`); `.couche-viz { overflow-x: clip }` so the left face never adds horizontal scroll on narrow viewports; a `prefers-reduced-motion` guard re-asserting `opacity: 1` (the global rule sets `animation-name: none`, which would otherwise leave the `li` opacity:0 base hidden).

**Colour (LOCKED, sky ramp, three tones):** top / front / side per layer -- Le Propos `#E8F6FE / #E8F6FE / #C8E9FC`; Le Plan `#DCF1FE / #C7EBFC / #A3DCF8`; La Construction `#B6E4FB / #93DAF9 / #66C7F1`; Les Pièges Anglais `#74CFF4 / #54C3F2 / #2CA8E5`; La Musique `#3FBBEC / #1FAEE8 / #0A8BC6`. Every face is blue; no white/transparent anywhere. Coral removed from THIS surface only; the global `--accent` coral token is untouched.

**Ink + AA (LOCKED):** every layer uses one dark ink `#1F2937` for both eyebrow and phrase -- zero white text. All five sky fills are light enough that dark ink clears 4.5:1 (deepest front `#1FAEE8` vs `#1F2937` ~ 5.8:1).

**Typography (LOCKED):** phrases Inter 800 (extrabold) 16px; eyebrows DM Mono uppercase 11px, 0.14em tracking. Both ride the `skewY(-14deg)` stack so the type slants on the isometric plane. Fonts wired to our loaded faces (`var(--f-en)` / `var(--f-mono)`), not the literal family names.

**Copy (placeholder, Chadi refines on preview):** the five How-to phrases + the section heading/intro are first-pass copy. No em-dashes anywhere.

**Correctives (history):**
- *Corrective 1:* rebuilt v1 (flat tilted plane) into a true isometric stack with the two pseudo-element depth faces + per-layer z-index.
- *Corrective 2:* moved the headline onto the slanted plane; removed eyebrows + detail.
- *Corrective 3 (sky):* sky ramp + single dark ink, flush (gap 0), rounded 7px, phrases Inter 800, couche eyebrows re-added in DM Mono uppercase.
- *Corrective 4 (sky):* (1) text back ON the plane -- removed the inverse-skew wrapper so eyebrow + phrase slant with the box; (2) three-tone all-blue faces -- top-lit gradient front, blue side/top faces, no white/transparent (there was no inset-white box-shadow; the perceived white was the pale top tint); (3) z-index lower-layers-in-front so no label is occluded.
- *Corrective 5 (SVG rebuild):* replaced the CSS build with one `<svg>` of exact polygons (verified coordinates) -- continuous left wall, square corners, blue-only faces, real `<text>` on the plane. Phrase 16px so the longest fits the narrowest face.
- *Corrective 6 (uiverse skewed stack):* replaced the SVG with Chadi's uiverse skewed-stack component (CSS `skewY` stack + `::before`/`::after` 3D faces, `li:hover` translateX). Wired our strings + sky tokens + fonts; `overflow-x: clip` + reduced-motion guard. Fixed 360px li width (no fan; centered). NOTE at that point: per-block top faces sat on every li.
- *Corrective 7 (overlap geometry):* per-block top faces removed; real overlap (`li + li { margin-top: -22px }`) so each couche hides the bottom of the one above; top face only on the first couche; depth 20 -> 22px; min-height 70 -> 72; content top-aligned so the hidden strip stays empty.
- *Corrective 8 (this PR -- two edge fixes):* (1) Defect 1 (right-edge notch): locked every couche to one identical width (`width: 360px; box-sizing: border-box; flex: 0 0 auto`, phrases `overflow-wrap: anywhere` to wrap rather than widen). NOTE: in the current overlap build the widths were already uniform (measured 360px, right edges aligned) and an element-probe found ZERO background gaps between couches on the right -- the right edge is already one continuous line; the notch Chadi circled was the pre-overlap (corrective-6) render. The width-lock is applied as the prescribed fix + a guard against any font-driven widening on the deployed build. (2) Defect 2 (bottom-left flap): the bottom couche's exposed left face had its lower-left corner pushed past the block silhouette by the skew; clipped it with `clip-path` on `li:last-child::before` so the left wall ends cleanly at the front-face bottom (verified gone in a render).

**F-ID note:** F-483 is this same feature (a reskin, not a new ticket), so the F-ID is unchanged. F-483 was the next free ID after F-482 when created (BACKLOG ceiling F-482, merged `f769cf5`/PR #16; PRD does not track F-4xx). FE+BE share the F-namespace; this FE-root BACKLOG is canonical for both, and no other `F-483` exists.
