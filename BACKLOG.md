...
**Status:** Committed to `main` locally (HEAD), push HELD pending Chadi confirm (FE auto-deploys to Vercel prod). SHA reported to Chadi at hand-off; this line gets the final SHA on Shipped flip. After push + CI green: prod spot-check `/carte` renders the serpentine, islands load (no 404), current node in view, no 375px overflow, then flip to Shipped.
