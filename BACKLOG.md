...
**Decisions (Chadi, 2026-06-16):** (1) Build now against the expected asset path, drop the clean PNG after, tune hotspots in the visual gate. (2) Labels are NOT baked into the image -- overlays render the per-island label pills. (3) The render shipped WITHOUT the coral path -- the FE draws it as an SVG ribbon through the hotspots in journey order (completed solid, upcoming muted).
