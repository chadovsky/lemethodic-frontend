...
## F-484 -- FE: La Carte backbone scaffold (Persona-as-Config, content-free)

**Status:** In progress -- branch `feat/f-484-carte-backbone-scaffold` off `main` @ `6472f8a`. Local gates: unit 595/595 (vitest, +13 new persona tests, zero regressions), `tsc --noEmit` clean for all touched files (3 pre-existing errors in untouched files: `app/(shell)/layout.tsx`, `mdx-components.tsx`, `playwright.config.ts`). F-225 receipts captured by `tests/e2e/f-484.spec.ts` (`f-484-carte-{1440,375}.png` + trace `f-484.zip`, gitignored per accepted debt). CI + Vercel branch preview gate the merge; Chadi gates the squash personally.

**ID note:** FE BACKLOG ceiling was F-482; F-483 appears free in this BACKLOG, but the dispatch + amendment explicitly claimed **F-484** (a parallel-sidepanel F-483 is the likely reason). BE `master` ceiling unreachable (network down to github.com at build time). To finalize at push: `git fetch` + grep both BACKLOG ceilings and confirm F-483 is not orphaned before the PR.

**Why:** the journey backbone must be expressible as config so new personas (TEF, DELF, DALF, Business French, Hobby, AP French) ship as config rows, never as new screens or flow. Two invariants: (1) the backbone runs with every slot bientot and lights up slot by slot; (2) a new persona ships as config only. This scaffold stands the backbone up content-free, with TCF as the first (and only journey-bound) persona.

**Amendment that resolved the carte conflict (Chadi, in-session):** the dispatch's "every cell bientot on /carte" contradicted the green F-472 specs (education `current` + the single `carte-current-cta`). Resolution: (a) carte cell state is config + progress driven, never hardcoded bientot; (b) TCF keeps its real states and theme-keyed `/ile/<theme>` routes, so those F-472 assertions stay green unchanged; (c) **no-gates doctrine** -- the carte has no `locked`/Verrouillé state, so the F-472 locked-cell assertions are dead-model and were updated to bientot (legitimate surface change, not cascade breakage); (d) invariant 1 (all-bientot) is proven by the STUB persona in the config-only proof, not by blanking TCF.

**Built (2026-06-25):**
- **Persona config layer (`lib/personas/{types,tcf,stub,index}.ts`, new):** typed product config, NOT localStorage. `types.ts` -- `Level` (re-exported from journey, single source), `Skill`, `MethodId`/`MethodScope`/`MethodStatus`, `Theme` ({id,label,order,pins,query?,excludes}), `Station`, `Persona`. `tcf.ts` -- the TCF seed (exam, 5-level band, the real 7 journey themes, full method registry all `bientot`, terminus `mock`, `journeyBound`). `index.ts` -- a `PERSONA_REGISTRY` keyed by id; `getActivePersona()` reads the active id from the `lm.activePersona.v1` seam (selection is user state, the config is not); `stationsFor()` generates (theme x level x cell-method) stations; `cellStatusFor()` bridges the journey/progress seam (real states on the learner's resolved-level column only, model `locked` -> bientot per no-gates); `recommendNext()` (Le Cap) + `readiness()` stubs return null / 0.
- **Carte 2D grid (`components/carte/CarteTable.tsx`, evolved):** rows = the active persona's themes (in order) + a grammar foundation row for journey-bound personas; columns = the persona's levelBand. Each cell is a (theme x level) station summary: completed -> done chip, current -> the single CTA, otherwise an inert bientot slot. A non-journey persona renders every cell bientot with zero component change. Preserves the full DOM contract (`carte-journey`/`carte-map`/`carte-level`/`carte-grammar`/`carte-ile`/`carte-current-cta`, table at >=640, cards at <640). Per-theme palette via `colorForTheme` (canonical themes mapped, molds cycle `CARTE_PALETTE`); NO coral, indigo NIVEAU pill.
- **Shells (`components/carte/{BientotSlot,LeCapSlot,GlobalRail,ReadinessSignal,CarteViewToggle}.tsx`, new):** `BientotSlot` -- the one reusable inert slot (visible, notify-me affordance, compact + full forms). `LeCapSlot` -- top-of-carte session-plan slot, reads `recommendNext` (null -> coming-soon). `GlobalRail` -- SRS/Mock/Reading global methods, all bientot, with the readiness signal. `ReadinessSignal` -- always-visible `X %` (0 now), never a gate. `CarteViewToggle` -- theme|skill twin view, defaults to theme so the contract resolves on load; skill view groups the same station set by CO/CE/EO/EE (bientot skeleton).
- **Table-data (`lib/carte/table-data.ts`):** added `CARTE_PALETTE` (the 8 carte colours, no coral) + `colorForTheme(id, order)`.
- **Docs:** `ARCHITECTURE.md` gained the "Journey Backbone (Persona-as-Config)" section (entities, two data layers, mastery/SRS, derived views, no-gates/readiness, bientot, Le Cap). `SITEMAP.md` gained the "Persona-driven surfaces" section.

**Tests:** `tests/unit/personas/personas.test.ts` (new, 13) -- registry + active selection, TCF shape, `stationsFor` count, `cellStatusFor` (real states on resolved level only, locked->bientot, progress advance, all-bientot for the mold persona), Le Cap/readiness stubs, the stub fixture shape. `tests/unit/carte/CarteTable.test.tsx` -- the locked-cell assertions updated to bientot (no-gates); all other assertions unchanged and green. `tests/e2e/f-484.spec.ts` (new) -- the 7x5 TCF grid + shells + twin view, the config-only proof (swap to the stub persona via `lm.activePersona.v1` -> 3x2 all-bientot grid, zero CTAs, no code change), and the 375 stacked-card / no-overflow case with receipts. `tests/e2e/f-472.spec.ts` -- the locked famille assertion updated to the bientot slot (no-gates); the rest of the contract unchanged.

**Scope held:** the seance walkability gate (F-460 empty state) is untouched -- the journey model keeps `locked` for the seance; only the carte presentation maps it to bientot. No content authored. No level-scoped routes. The stub persona is a proof fixture, never surfaced in any UI.

## F-483 -- FE: 5-couche methodology visualizer on /la-methode (isometric 3D stack)

**Status:** **Shipped** -- squash-merged to main `8b956d9` (PR #19), rebased onto `ad5de74` (F-484 / PR #18) first. CI green (unit + e2e) on the PR before merge. The isometric (skewY 3D coral stack) shipped as-is; the brief's flat-v3 escape hatch was NOT taken (kept the as-built, intentionally sourced treatment) and remains a one-commit follow-up if Chadi judges it unprofessional on the live preview. Copy + the exact coral ramp are still placeholders for Chadi to refine. Build green (`✓ Compiled successfully`, 108/108 pages). Em-dash sweep over all F-483-authored comments + test titles (the only remaining em-dash on `/la-methode` is the pre-existing `LessonList` subtitle copy, untouched per scope). F-225 receipts `tests/screenshots/f-483-la-methode-{1440,375}.png` + trace `f-483.zip` (gitignored).

> ID-correction note: the dispatch referenced "PR #17"; that PR was the closed/abandoned `feat/f483-couche-visualizer-sky` (blue reskin), not this coral/v3 line. This entry shipped as PR #19 off `feat/f-483-couche-visualizer`. PR #17 stays closed.
...
**Scope:** An isometric 3D stack (the uiverse skewY effect, by andrew-demchenk0, MIT) showing the five couches as extruded blocks, reskinned to v3. Each block carries ONE short How-to headline (Inter) that lies ON the slanted face -- it inherits the wrapper skew (NOT counter-skewed), so the type sits on the isometric plane like the reference. No couche-name eyebrows, no detail on the face. The geometric skew is intentional for this one component and overrides the general no-sharp-corners default (Chadi sourced the effect). The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.

**Built:**
- **`components/la-methode/CoucheStack.tsx` (new, server component):** section h2 + intro, then a `role="list"` of five blocks. Each block carries one `<h3>` headline (`data-testid="couche-stack-headline"`) and sets `--face-top` (face ramp token) + `--face-text` (AA-safe black/white); the `<li>` sets `--i` (z-index). Static text (not interactive): screen readers read the five h3 headlines under the section h2.
- **Heading-order fix:** `LessonList` + `LessonListContainer` gained an optional `methodology` slot rendered between the page h1 header and the lesson sections; `app/(shell)/la-methode/page.tsx` passes `<CoucheStack />` into it. The h1 + subtitle stay in `LessonList` (ecole-list e2e unchanged); the visualizer h2 now sits directly under the h1.
- **`app/globals.css` (F-483 block):** coral ramp tokens `--couche-ramp-1..5`, `--couche-skew (-14deg)`, `--couche-depth`, `--couche-lift`, and the `.couche-stack*` styles -- the wrapper skewY tilt; each layer's two depth faces via `::before` (left, skewY 45deg origin right) + `::after` (back/top, skewX 45deg origin bottom), darker-shaded with `color-mix(... black)`; per-layer z-index (+ a `:has()` hover bump); the headline lies on the plane (inherits the wrapper skew, no counter-skew) with `white-space: nowrap` so it never wraps/clips; a decorative hover-lift; `@media (max-width:640px)` flat upright fallback (drop skew + faces, full width, headlines may wrap, no overflow); a `prefers-reduced-motion` guard. uiverse MIT attribution included. Only new hex is the coral ramp; depth faces use the `black` keyword in `color-mix`, text uses the `black`/`white` keywords (mode-stable).

**Palette (placeholder, Chadi refines on preview):** stepped (not blended) coral ramp, light at the top block to deep at the bottom: `#FAD4CB / #F4A28F / #ED7259` (dark text) then `#C43E26 / #9E3019` (light text). Each block's two depth faces are a darker shade of its top face (left face 36% darker, top face 20% darker, computed via `color-mix`). Whether Les Pièges Anglais gets standout emphasis is deferred to the preview pass.

**AA contrast (resolved):** the headline clears 4.5:1 on every block. Upper three blocks take dark text (10.8 / 7.3 / 5.0:1), lower two take light text (5.2 / 7.3:1). Chadi can flip to all-dark-with-lighter-deep-steps on the preview if he prefers; both directions stay >=4.5:1.

**Copy (placeholder, Chadi refines on preview):** the five How-to headlines + the section heading/intro are first-pass copy, each headline sized to one line. No em-dashes anywhere. The original per-couche detail lines were removed from the face (corrective 2); they can return as a below-stack hover caption (upright, off the plane) if Chadi wants them -- logged, not built.

**Correctives (this branch):**
- *Corrective 1:* v1 applied only the skewY tilt with a flat coral gradient and no depth faces, so it read as a flat tilted plane. Rebuilt as a true isometric 3D stack: two pseudo-element depth faces per layer, per-layer z-index, a stepped (AA-tuned) ramp with darker side faces, increased depth, and the heading-order fix.
- *Corrective 2:* the text was counter-skewed upright (per the prior brief) so it floated flat across the slanted faces. Reversed: the headline now lies ON the plane (inherits the wrapper skew). Eyebrows deleted (removes DM Mono); detail removed from the face; one short line per block, `nowrap`, sized so nothing wraps or clips.
