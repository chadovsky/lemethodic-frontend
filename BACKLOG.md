...
## F-483 -- FE: 5-couche methodology visualizer on /la-methode (SVG isometric block, sky)

**Status:** Built on `feat/f483-couche-visualizer-sky`. PR #17 against `main`; NOT merged -- Chadi judges on the Vercel preview and confirms before any squash-merge. Local gates: full unit suite (incl. `CoucheStack` suite, 7); clean `pnpm build` (fresh `rm -rf .next`); f-483 + ecole-list e2e green; `tsc` clean for the component. F-225 receipts `tests/screenshots/f-483-la-methode-{1440,375}.png` + trace `f-483.zip` (gitignored).
...
**Scope (current = SVG rebuild):** One `<svg viewBox="0 0 680 520" role="img">` (title + desc) of exact polygons showing the five couches as one isometric 3D block. The CSS skew + pseudo-element build was replaced because it left a white wedge, soft corners, and seam leaks; polygons make every edge crisp and leave no canvas gap by construction. All five left edges align at x=140 (the fan grows each lower box rightward, so the left side is one clean wall). Each couche is a three-tone blue block: front face (front tone), continuous left side wall (side tone), and a top cap on couche 0 (top tone) -- every face blue, no white/transparent. SVG `<text>` rides the same -0.2 shear so the DM Mono eyebrow + Inter extrabold phrase lie ON the isometric plane. Square corners (deliberate exception to the rounded-only rule -- a 3D block needs crisp edges). The page h1 ("La Méthode") leads; the visualizer is the h2 section right under it, then the 27 lessons.

**Built:**
- **`components/la-methode/CoucheStack.tsx` (SVG):** section h2 + intro, then one responsive `<svg width="100%" viewBox="0 0 680 520" role="img">` with `<title>` + `<desc>` (the desc lists all five couches + phrases for screen readers, since `role="img"` collapses the inner nodes). Geometry is computed from parameters (k=0.2 shear, depth dx=-20/dy=-14, X0=140, Y0=120, H=72, P=72, W_i=300+26i) in a helper, output verified to match the brief's exact coordinates. Draw order: 5 side-wall polygons, couche-0 top cap, 5 front faces (left edge nudged to x=139 so AA leaves no hairline), then 5 text groups `<g transform="matrix(1,-0.2,0,1,162,TY)">` each with an eyebrow `<text data-testid="couche-stack-eyebrow">` + phrase `<text data-testid="couche-stack-headline">`. Fills come from the sky tokens via `fill: var(--couche-...)` -- no raw hex in the component.
...
- **`app/globals.css` (F-483 block):** reduced to the colour identity only -- `--couche-top-1..5` (top/cap tones) + `--couche-ramp-1..5` (front tones) + `--couche-side-1..5` (side/wall tones) + `--couche-ink (#1F2937)`. All the prior `.couche-*` geometry CSS (skew wrapper, pseudo-element faces, flush stack, media fallback) is deleted -- the SVG owns geometry now. The SVG is responsive (`width:100%`), so no separate mobile fallback; it scales down on small screens.
...
**Typography (LOCKED):** phrases Inter 800 (extrabold), 16px + -0.01em tracking (the brief's ~17px overhung the narrowest top face by 17px at the locked geometry/copy; 16px fits the longest phrase inside couche 0 with ~6px margin -- see corrective 5); eyebrows DM Mono uppercase, ~0.14em tracking, ~11px. SVG `<text>` slants on the plane via the -0.2 shear matrix.
...
- *Corrective 5 (this PR -- SVG rebuild):* replaced the whole CSS skew/pseudo-element build with one `<svg>` of exact polygons (geometry verified against the brief's coordinates). Kills the white wedge, soft corners, and seam leaks by construction: continuous left wall (no notches/canvas gaps), square corners, blue-only faces, real `<text>` on the plane. The CSS F-483 block is now colour tokens only. Phrase dropped 17px -> 16px so the longest phrase fits inside the narrowest (top) face at the locked geometry; the SVG scales responsively (text smaller on mobile -- flagged).
