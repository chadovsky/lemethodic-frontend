...
test.describe('App shell — desktop (1280×800)', () => {
  ...
  test('navigating carte→la-methode shifts active state', async ({ page }) => {
    await page.goto('/carte')
    await expect(page.getByTestId('sidebar-link-carte')).toHaveAttribute('aria-current', 'page')
    ...
    await expect(page.getByTestId('sidebar-link-carte')).not.toHaveAttribute('aria-current', /.+/)
  })

  test('shell renders on /carte with sidebar and 6 nav links', async ({ page }) => {
    await page.goto('/carte')
    await expect(page.getByTestId('app-shell-sidebar')).toBeVisible()
    await expect(page.getByTestId('sidebar-link-seance')).toBeVisible()
    await expect(page.getByTestId('sidebar-link-carte')).toBeVisible()
    await expect(page.getByTestId('sidebar-link-la-methode')).toBeVisible()
    await expect(page.getByTestId('sidebar-link-bibliotheque')).toBeVisible()
    await expect(page.getByTestId('sidebar-link-examen')).toBeVisible()
  ...
  test('active state is on "Tableau de bord" when visiting /carte', async ({ page }) => {
    await page.goto('/carte')
    await expect(page.getByTestId('sidebar-link-carte')).toHaveAttribute(
      'aria-current',
      'page',
    )
  ...
  test('clicking "Compte" navigates to /profil and active state shifts', async ({ page }) => {
    ...
    await expect(page.getByTestId('sidebar-link-carte')).not.toHaveAttribute(
      'aria-current',
      /.+/,
    )
  })
  ...
  test('shell wraps the /carte page with a heading', async ({ page }) => {
    await page.goto('/carte')
...
test.describe('App shell — mobile (375×667)', () => {
  ...
  test('no horizontal overflow on /carte', async ({ page }) => {
    await page.goto('/carte')
