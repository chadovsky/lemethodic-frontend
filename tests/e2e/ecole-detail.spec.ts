...
test.describe("La Méthode lesson detail — desktop (1280×800)", () => {
  ...
  test('clicking next on /la-methode/3 navigates to /la-methode/4', async ({ page }) => {
    await page.goto('/la-methode/3')
    await page.getByTestId('lesson-nav-next').click()
    await expect(page).toHaveURL(/\/la-methode\/4$/)
  })

  test('/la-methode/1 — no previous button, next links to /la-methode/2', async ({ page }) => {
    await page.goto('/la-methode/1')
    await expect(page.getByTestId('lesson-nav-prev')).toHaveCount(0)
    await expect(page.getByTestId('lesson-nav-next')).toHaveAttribute('href', '/la-methode/2')
  })

  test('/la-methode/27 — no next button, prev links to /la-methode/26', async ({ page }) => {
    await page.goto('/la-methode/27')
    await expect(page.getByTestId('lesson-nav-next')).toHaveCount(0)
    await expect(page.getByTestId('lesson-nav-prev')).toHaveAttribute('href', '/la-methode/26')
  })

  test('/la-methode/99 — renders Next.js 404', async ({ page }) => {
    const response = await page.goto('/la-methode/99')
  ...
  test('lesson card #5 on /ecole resolves to /la-methode/5 detail page', async ({ page }) => {
    // Also mock the list endpoint for this test
    await page.route('**/api/la-methode/lessons', async (route) => {
  ...
  test('audio player shows 15 waveform bars on /la-methode/3', async ({ page }) => {
    await page.goto('/la-methode/3')
    await expect(page.getByTestId('lesson-waveform-bar')).toHaveCount(15)
  })

  test('ArrowRight on /la-methode/3 navigates to /la-methode/4', async ({ page }) => {
    await page.goto('/la-methode/3')
    await page.keyboard.press('ArrowRight')
    await expect(page).toHaveURL(/\/la-methode\/4$/)
  })

  test('ArrowLeft on /la-methode/3 navigates to /la-methode/2', async ({ page }) => {
    await page.goto('/la-methode/3')
    await page.keyboard.press('ArrowLeft')
    await expect(page).toHaveURL(/\/la-methode\/2$/)
  })

  test('ArrowLeft on /la-methode/1 does not navigate away (boundary guard)', async ({ page }) => {
    await page.goto('/la-methode/1')
    await page.keyboard.press('ArrowLeft')
    await expect(page).toHaveURL(/\/la-methode\/1$/)
  })
})
...
test.describe("La Méthode lesson detail — mobile (375×667)", () => {
  ...
  test('no horizontal overflow on /la-methode/3', async ({ page }) => {
    await page.goto('/la-methode/3')
