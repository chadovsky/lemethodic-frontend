...
The rename migration is **substantial** — touches routes, copy, internal API field names where they were named after the legacy concepts (`tache_rubric`, `couche_score`, etc., audit needed), the PRD ticket specs in `73d898f`, BACKLOG.md entries, and the soft-beta visual capture baseline. **PRODUCT.md is now the source of truth; the codebase and PRD will catch up via M-RENAME.**
